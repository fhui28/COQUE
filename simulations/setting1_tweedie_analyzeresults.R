...
library(mvtnorm)
library(ROCR)
here::i_am("simulations/setting1_tweedie_analyzeresults.R")
library(here)
...
N_seq <- c(100,200)
num_datasets <- 500
methods <- c("backward_elimination", "COQUE")
...
for(k1 in 1:2) { for(k0 in 1:num_datasets) {
    filename <- paste0("setting1_tweedie_n", N_seq[k1], "_dataset", k0, ".RData")
...
#' # Examine results
##-----------------------
apply(all_RMSE[,,c("COQUE", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_RMSE[,,c("COQUE", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_F1[,,c("COQUE", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_F1[,,c("COQUE", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_MSE_uniquevalues[,,c("COQUE", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_MSE_uniquevalues[,,c("COQUE", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_time_taken[,,c("COQUE", "backward_elimination")]/60, c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_time_taken[,,c("COQUE", "backward_elimination")]/60, c(1,3), sd, na.rm = TRUE) %>% round(3)


apply(all_sensitivity[,,c("COQUE", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_sensitivity[,,c("COQUE", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_specificity[,,c("COQUE", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_specificity[,,c("COQUE", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_accuracy[,,c("COQUE", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_accuracy[,,c("COQUE", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)
