...
#' abstract: Binary responses. Note the code was also set up to run as an array job on a HPC.
...
library(mvtnorm)
library(ROCR)
here::i_am("simulations/setting1_binary_analyzeresults.R")
library(here)
...
rm(true_G, true_Sigma, true_ranef_cov, num_clus)
...
for(k1 in 1:2) { for(k0 in 1:num_datasets) {
    filename <- paste0("setting1_binary_n", N_seq[k1], "_dataset", k0, ".RData")
    ...
    if(inherits(try(sapply(glmm_backwardelim[1:num_resp], function(x) x$coefficients), silent = TRUE), "try-error"))
        next;
    if(is.na(glmmPen_fit$resp1[1]))
        next;
    if(inherits(try(sapply(rpql_fit[1:num_resp], function(x) x$fixef), silent = TRUE), "try-error"))
        next;
    ...
    all_F1[k1,k0,4] <- ROCR::performance(makepred_obj, measure = "f")@y.values[[1]][2]
    all_RMSE[k1,k0,4] <- sqrt(mean(cw_coefficients[,-1] - true_fixed_effects[,,k1][,-1])^2)
    all_MSE_uniquevalues[k1,k0,4] <- mean((cw_uniquevalues_nointercept - true_distinct_values_nointercept[,k1])^2)
    ...
    all_F1[k1,k0,5] <- ROCR::performance(makepred_obj, measure = "f")@y.values[[1]][2]
    all_RMSE[k1,k0,5] <- sqrt(mean(cw_coefficients[,-1] - true_fixed_effects[,,k1][,-1])^2)
    all_MSE_uniquevalues[k1,k0,5] <- mean((cw_uniquevalues_nointercept - true_distinct_values_nointercept[,k1])^2)
...
#' # Examine results
##-----------------------
apply(all_RMSE[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_RMSE[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_F1[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_F1[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_MSE_uniquevalues[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_MSE_uniquevalues[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_time_taken[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")]/60, c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_time_taken[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")]/60, c(1,3), sd, na.rm = TRUE) %>% round(3)


apply(all_sensitivity[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_sensitivity[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_specificity[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_specificity[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)

apply(all_accuracy[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), mean, na.rm = TRUE) %>% round(3)
apply(all_accuracy[,,c("COQUE", "glmmLasso", "glmmPen", "rpql", "backward_elimination")], c(1,3), sd, na.rm = TRUE) %>% round(3)
