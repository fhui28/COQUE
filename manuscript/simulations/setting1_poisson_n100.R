...
rm(list = ls())
here::i_am("simulations/setting1_poisson_n100.R")
library(here)
...
dosims <- function(NAI) {
    ...
    ##------------------
    #' # Stacked penalized GLMMs using the [glmmPen::glmmPen()] package.
    ##------------------
    tic <- proc.time()
    glmmPen_fit <- try(stacked_glmmPen(formula = resp ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12 + x13 + x14 + x15 + (1 + x1 + x2 + x3 + x4 | id),
                                       y = simy$y[,-ncol(simy$y)],
                                       data = dat,
                                       num_cores = num_resp,
                                       response_type = response_type),
                       silent = TRUE)
    toc <- proc.time()
    if(!inherits(glmmPen_fit, "try-error"))
        glmmPen_fit$time_taken_full <- toc - tic


    ##------------------
    #' # Stacked penalized GLMMs using the [rpql::rpql()] package.
    ##------------------
    tic <- proc.time()
    rpql_fit <- try(stacked_rpql(y = simy$y[,-ncol(simy$y)],
                                 data = dat,
                                 response_type = response_type,
                                 num_fixef = ncol(true_fixed_effects) - 1,
                                 formula_Z = "(x1 + x2 + x3 + x4 | id)",
                                 lambda_seq = lseq(1e-8,1,length=100),
                                 num_cores = num_resp),
                    silent = TRUE)
    toc <- proc.time()
    if(!inherits(rpql_fit , "try-error"))
        rpql_fit$time_taken_full <- toc - tic
    ...
    save(penalized_fit,
         ...
         rpql_fit,
         simy,
         file = paste0("setting1_poisson_n", num_clus, "_dataset", NAI, ".RData"))
  }
