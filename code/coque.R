...
#' @param num_cores The regularization path is constructed via parallel computing, and this argument controls the number of cores used. Defaults to \code{NULL}, in which case it is set to \code{detectCores() - 2}. 
#' @param control A list containing elements controlling the maximum number of iterations in the algorithm (\code{maxit}), the convergence criterion (the norm of the difference between estimates from successive iterations must be smaller than this value; \code{eps}), a tolerance to round values to zero (this is technically not needed as COQUE and the broken adaptive ridge penalty will produce exactly zero estimates up to machine error, but is included anyway; \code{round_eps}), whether to use replace the sandwich information matrix with the observed information (i.e., the bread) matrix if instabilities are detected in the former (\code{fallback_to_bread}), and whether to print updates along the way (\code{trace}).
#' 
#' 
#' @return An object of class "coque" with the following elements (and not necessarily in the order below):
...
#' \item{ICs:}{A matrix of some information criteria and their values on the regularization path. The number of rows is equal to \code{length(lambda)}. The current information criteria calculated include: Akaike information criterion (AIC) with model complexity penalty of 2; Bayesian information criterion (BIC) with model complexity penalty of \code{log(num_clus)} where \code{num_clus} is the numbers of clusters in the multivariate GLMM; Extended Bayesian information criterion (EBIC) with model complexity penalty of \code{log(num_clus) + log(penalized parameters)}; Extended Bayesian information criterion (EBIC2) with model complexity penalty of \code{log(num_clus) + 2 * log(penalized parameters)}.}
#' \item{time_taken:}{The time taken to construct the regularization path.}
#' \item{num_resp/num_clus/response_type/response_names/fixed_effects_names:}{Additional auxilary function that can be safely ignored.}
...
coque <- function(object, nlambda = 100, lambda_min_ratio = 1e-6, lambda = NULL, min_df = 0, num_cores = NULL,
                    control = list(maxit = 1000, eps = 1e-6, round_eps = 1e-6, fallback_to_bread = TRUE, trace = FALSE)) {
   ...
   #     sandwich = (solve(sandwich_matrix)/num_clus) %>% diag %>% sqrt %>% {.[1:(16*6)]})
    ...
    reparametrized_fixed_effects_matrix <- matrix(reparametrized_psihat[1:length(fixed_effects_matrix)], nrow = num_resp, byrow = TRUE)
    additional_weights <- matrix(1/abs(colSums(reparametrized_fixed_effects_matrix[-1,])), nrow = num_resp, ncol = ncol(fixed_effects_matrix), byrow = TRUE) 
    ...
    out$call <- match.call()
    out$num_resp <- num_resp    
    out$num_clus <- num_clus    
    out$response_type <- object$response_type    
    out$response_names <- names(object)[1:num_resp]  
